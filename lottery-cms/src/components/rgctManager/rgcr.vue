              ...
              <div v-show="ruleForm.type.value && hasTypeSearched">
                ...
                <div class="tl content-row">
                  ...
                  <el-radio v-model="indicatorSelected" label="2" style="margin-top:10px">
                    <span>自定义</span>
                    <el-input v-model="ruleForm.amountCustom" style="width:150px" type="number" @focus="ammountCustomChange" @change="acChange()"></el-input>
                    <span>元 =</span>
                    <el-input v-model="ruleForm.indicatorCustom" style="width:150px" type="number" @focus="indicatorCustomChange" @change="icChange()"></el-input>
                    <span>倍</span>
                  </el-radio>
                </div>
                <!-- <div class="tl content-row">
                ...
                <div class="tl content-row clearfix">
                  <el-button type="primary" @click="recharge()" class="yes fr">确认存入</el-button>
                </div>
              </div>
            </div>
          </div>
  ...
  export default {
    ...
    methods: {
      ...
      recharge() {
        ...
        if (vm.indicatorSelected == '2') {
          if (!positiveDecimal(vm.ruleForm.amountCustom)) {
            this.$message({
              message: '请输入自定义打码量金额',
              type: 'error',
              duration: vm.duration,
          ...
          if (!positiveDecimal(vm.ruleForm.indicatorCustom)) {
            this.$message({
              message: '请输入自定义打码量倍数',
              type: 'error',
              duration: vm.duration,
      ...
      ammountCustomChange(event) {
        console.log('ammountCustomChange', event);
        this.acFocus = true;
        this.icFocus = false;
      },
      indicatorCustomChange(event) {
        console.log('indicatorCustomChange', event);
        this.icFocus = true;
        this.acFocus = false;
      },
      acChange(value) {
        console.log('acChange', value);
      },
      icChange(value) {
        console.log('icChange', value);
      }
    },
    watch: {
      'ruleForm.amount' () {
        if (this.ruleForm.amount) {
          if (this.indicatorSelected == 1) {
            this.ruleForm.amountDefault = this.ruleForm.amount * this.ruleForm.indicatorDefault;
          } else if (this.indicatorSelected == 2) {
            if (this.ruleForm.amountCustom) {
              this.ruleForm.indicatorCustom = parseFloat(this.ruleForm.amountCustom / this.ruleForm.amount).toFixed(2);
            } else if (this.ruleForm.indicatorCustom) {
              this.ruleForm.amountCustom = this.ruleForm.indicatorCustom * this.ruleForm.amount;
            }
          }
        } else {
      ...
      'ruleForm.amountCustom' () {
        if (this.ruleForm.amount) {
          if (this.acFocus && !this.icFocus) {
            if (this.ruleForm.amountCustom) {
              if (this.ruleForm.amount) {
                if (parseFloat(this.ruleForm.amountCustom / this.ruleForm.amount) < 0.01) {
                  this.ruleForm.indicatorCustom = '0';
                } else {
                  this.ruleForm.indicatorCustom = parseFloat(this.ruleForm.amountCustom / this.ruleForm.amount).toFixed(
                    2);
                }
              } else {
                this.ruleForm.indicatorCustom = '0';
