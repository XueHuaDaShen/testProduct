  ...
  <div class="lottery-wrap" v-loading="loaded">
      ...
      <div class="header-right">
        <div class="header-right-">
          ...
          <div class="history-lottery-number" v-if="issueArr.toString()!==''">
            <span class="dice animated" :class="refreshIssue?'flip dice'+k:'dice'+k" v-for="(k, i) in issueArr[0].luck_no.split(',')" :key="i.toString()"></span>
          </div>
        </div>
        <div class="explain">
          <span>
    ...
    <div style="width:100%;background:#f4f4f4;">
      <div class="lottery-content">
        <div class="lottery-top" style="min-height:496px;">
          <div class="lottery-num">
            <lotteryNumberArr v-if="isShowLotteryNum" type="ks" :title="betsArr.title" :parTitle="groupName" :gameid="gameid" :issueArr="issueArr" :showPrize="false"></lotteryNumberArr>
          </div>
          <div class="bets-wrap-parent">
            <div class="bets-wrap">
              ...
              <ksBetsArea :type="ksType" :ksPrize="c_prize" :userRefund="userRefund" :adjust="adjust" :diff="diff" :maxRefund="maxRefund" :moneyType="moneyType" v-if="randerbetsAreaTen" @sendBetsArr="handlesendBetsArr" :betsArr="betsArr"></ksBetsArea>
            </div>
          </div>
...
<script>
  ...
  import ksBetsArea from '../../bets/ksBetsArea';
  ...
  export default {
    ...
    data() {
      return {
        ...
        logoImgCode: '',
        ksType: 'andValue',
        ksTypeArr: ['andValue', 'sizeDouble', 'threeSame', 'twoSame', 'threeNoSame', 'twoNoSame', 'threeLink', 'singleDice'],
        isClick: false, // 双击
    ...
    created() {
      ...
      this.$store.dispatch('setbodyBG', 'ks');
      localStorage.setItem('bodyBG', 'ks');
    },
    beforeDestroy() {
    ...
    computed: {
      getBlance() {
        // console.log(typeof this.$store.state.blance)
    ...
    methods: {
      ...
      handleChangeNav(item) {
        ...
        try {
          ...
          if (this.userRefund > this.maxRefund) {
            prize = (2 * item.probability * (this.userRefund - this.diff)) / this.maxRefund;
            max_multiple = Math.floor(this.maxPrize / prize);
          } else {
            prize = (2 * item.probability * this.userRefund) / this.maxRefund;
            max_multiple = Math.floor(this.maxPrize / prize);
          }
          this.maxMultiple = max_multiple;
          this.c_maxMultiple = max_multiple;
          vm.prize = prize
          // console.log('item----',this.userRefund, this.maxRefund, this.diff, item.probability)
      ...
      handlesendBetsArr(data) {
        ...
        try {
          let prize, max_multiple;
          if (this.userRefund > this.maxRefund) {
            prize = (2 * data.probability * (this.userRefund - this.diff)) / this.maxRefund;
            max_multiple = Math.floor(this.maxPrize / prize);
          } else {
            prize = (2 * data.probability * this.userRefund) / this.maxRefund;
            max_multiple = Math.floor(this.maxPrize / prize);
          }
          this.maxMultiple = max_multiple;
          // console.log('data----',this.userRefund, this.maxRefund, this.diff, data.probability)
          // console.log(max_multiple)
          if (this.moneyType === '元') {
            vm.prize = prize
            vm.c_prize = prize
      ...
      getBetsNumber() {
        const vm = this;
        this.betsNumber = this.betsArr.data.join("|")
      },
      // 加入到购彩篮时
      handleJoin() {
        const vm = this;
        this.betsModeName = this.betsArr.title;
      ...
      handleDeleteBet(item, index, e) {
        e.stopPropagation();
        let t = this.groupName + ',' + this.betsType + ',' + this.betsArr.title;
        if (t === item.name) {
          this.betsArr.data = [];
          this.$store.dispatch('setBets', 0);
        }
      ...
      setInitData(data) {
        const vm = this;
        vm.navData = [];
        vm.lottery3id = data[0]._id;
        vm.groupName = data[0].name;
        vm.betsType = data[0].name;
        data[0].children[0].children.filter((v, vi) => {
          let o = {}
          o.play = v.note1; // 玩法
          o.plan = v.note2; // 方案
          o.prize = v.prize || 0; // 奖金
          o.probability = v.probability; // 赔率
          o.lotteryid = v._id; // lotteryid
          o.title = v.name;
          o.extra = v.extra || '';
          o.checked = false;
          o.ksType = vm.ksTypeArr[vi];
          o.data = [];
          vm.navData.push(o)
          // console.log(v)
        })
        // console.log(vm.navData)
        // vm.filterLotteryList(data[0].children, vm.twoStar);
      ...
      confirmLotteryFn() {
        ...
        if (allBets != 0) {
          ...
          if (vm.userBlance < totalMoney) {
            ...
            request.http(
              'post',
              '/lottery/chase/luckno', paramCryptFn({ chaseList, chaseDetail, sum }),
              (success) => {
                vm.isClick = false;
                // console.log(success);
                let code = success.returncode;
          ...
          if (vm.userBlance < totalMoney) {
            ...
            request.http(
              'post',
              '/lottery/issue/vote', paramCryptFn({ voteList }),
              (success) => {
                vm.isClick = false;
                // console.log(success);
                let code = success.returncode;
      ...
      gotoBetsFn() {
        ...
        vm.$alert(html, '确认信息', {
          ...
          if (vm.userBlance < vote_cash) {
            ...
            request.http(
              'post',
              '/lottery/issue/vote', paramCryptFn(data),
              (success) => {
                // console.log(success);
                let code = success.returncode;
                if (code === 103) {
