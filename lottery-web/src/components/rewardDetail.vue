              ...
              for (let i = 0; i < self.tables.items.length; i++) {
                ...
                for (let j = 0; j < children.length; j++) {
                  ...
                  for (let k = 0; k < child_.length; k++) {
                    ...
                    let child3 = {
                      name: child__.name,
                      refund: parseFloat(child__.refund).toFixed(2),
                      rowspan: 0,
                      children: []
                    };
                    ...
                    if (child__.extra && child__.extra != null) {
                      for (let v in child__.extra) {
                        ...
                        let child5 = {
                          name: parseFloat(child4.salary).toFixed(2),
                          rowspan: 1
                        };
                      ...
                      child4 = {
                        name: "",
                        salary: parseFloat(child3.refund).toFixed(2),
                        rowspan: 1,
                        children: []
                      };
                      child3.children.push(child4);
          ...
          for (let ii = rowspan.index; ii < children.columns; ii++) {
            ...
            if (parent) {
              templateStr +=
                ...
                '" style="border: 1px solid #DADADA;color: #000;line-height: 42px;">' +
                parent.name +
                "</td>";
              arrayRowspan[ii] = arrayRowspan[ii] + parent.childrenLength;
            } else {
              templateStr +=
                '<td style="border: 1px solid #DADADA;color: #000;line-height: 42px;">' +
                children.array[j].name +
                "</td>";
  ...
  mounted() {
    ...
    if (this.$route.query) {
      ...
      if (this.$route.query.id) {
        ...
        self.$alert(
          '<div class="lottery-title">该奖金组不存在，请联系管理员</div>',
          "系统提醒",
          {
