  ...
  <div>
    <div class="bets-text-wrap">
      ...
      <div v-if="lotteryType==='ssc'" class="show-betsNum-area" @drop="importBetsFile($event)">
        <textarea name="" placeholder="说明：
1、支持常见的各种单式格式，间隔符如： 换行符 回车 逗号 分号等
2、上传文件后缀必须是.txt格式,最大支持10万注，并支持拖拽文件到文本框进行上传
3、文件较大时会导致上传时间较长，请耐心等待！

格式范例：12345 23456 88767 33021 98897" v-model="betsNumber" ref="betsArea" @focus="handleTextareaFocus" @blur="handleTextareaBlur" @keyup="handleInputBetsNum" id="my-betsNumber-area" cols="30" rows="10"></textarea>
      </div>
...
格式范例：01 02 03|03 04 05|07 08 11" v-model="betsNumber" ref="betsArea" @focus="handleTextareaFocus" @blur="handleTextareaBlur" @keyup="handleInputBetsNum" id="my-betsNumber-area" cols="30" rows="10"></textarea>
      </div>
    ...
    <el-dialog title="温馨提示" :center="true" :visible.sync="dialogFormVisible">
      ...
      <textarea name="" v-model="filterNumber" cols="30" rows="10"></textarea>
