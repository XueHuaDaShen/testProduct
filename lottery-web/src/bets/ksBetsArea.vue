<template>
  <div class="ks-bets-wrap">
    <div class="ks-content" v-if="type === 'andValue'">
      <div class="rule">{{betsArr.plan}}
        <span>单注奖金：
          <i>{{setPrizeText(prize)}}</i>
        </span>
      </div>
...
<style lang="scss" scoped>
  ...
  .ks-bets-wrap {
    ...
    .size-double-bets {
      ...
      flex-wrap: wrap;
      div {
        line-height: 76px; // margin-left:30px;
      }
      span {
        display: block;
        width: 56px;
        height: 56px;
        line-height: 56px;
        border-radius: 50%; // font-family: STHeitiSC-Light;
        font-size: 32px;
        color: #4f4f4f;
        text-align: center;
