<template>
  <div class="pks-wrap">
    ...
    <div class="lottery-kind number" v-if="betsData.type!=='text' && betsData.type.indexOf('and')<0 && betsData.type!=='special' && betsData.type!=='sizeDouble' && betsData.type!=='interest' && betsData.type!=='tiger-end'&&betsData.type!=='ranking'&&betsData.type!=='pks-tiger-end'">
    ...
    <!-- 特殊号码 区 -->
    <div class="lottery-kind number pks-tiger-end" v-if="betsData.type==='pks-tiger-end'">
      <div class="betsList" v-for="(bets, bi) in betsData.betsTitle" :key="bi">
        <div class="split-bar"></div>
        <div class="bets-wrap">
          <strong class="bets-title">{{bets.charAt(1)==='位'?bets.charAt(0):bets}}</strong>
          <ul>
            <li v-if="bi!==vi" :class="checkInStyle(bi, v)" v-for="(v, vi) in tigerArr[bi]" :key="vi" @click="selectNum(bi, v, tigerArr[bi])">{{v.split('-')[0]+'龙'+v.split('-')[1]+'虎'}}</li>
          </ul>
        </div>
      </div>
    </div>
    <!-- 大小单双 区 -->
    <div class="lottery-kind number sizeDouble" v-if="betsData.type==='sizeDouble'||betsData.type==='ranking'">
      <div class="betsList" v-for="(bets, bi) in betsData.betsTitle" :key="bi">
        <div class="split-bar"></div>
        <div class="bets-wrap">
          <strong class="bets-title">{{bets.charAt(1)==='位'?bets.charAt(0):bets}}</strong>
